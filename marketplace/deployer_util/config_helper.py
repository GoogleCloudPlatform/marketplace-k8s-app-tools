    ...
    for role in self._roles:
      if role.get('rulesType') == 'PREDEFINED':
        ...
        for rule in role.get('rules', []):
          ...
          if not rule.get('apiGroups') or not list(filter(lambda x: x, rule.get('apiGroups'))):
            raise InvalidSchema("Missing or empty apiGroups in rules. "
                                "Did you mean [\"v1\"] or [\"*\"]?")
          if not rule.get('resources') or not list(filter(lambda x: x,
                                                     rule.get('resources'))):
            raise InvalidSchema('Missing or empty resources in rules.')
          if not rule.get('verbs') or not list(filter(lambda x: x,
                                                 rule.get('verbs'))):
