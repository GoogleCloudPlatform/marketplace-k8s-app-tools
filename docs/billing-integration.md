...
data:
  config.yaml: |
    ...
    # defined in "sources", or externally, i.e. by the app.
    # Since this metric is not supported by the "sources" section,
    # the app must send usage reports for this metric to the agent.
    - name: requests
      type: int
    ...
    # the app. The currently supported source is 'heartbeat', which
